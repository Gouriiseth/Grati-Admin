...
const DeleteAdminSuccess = ({isRole}) => {
    const style = {
        position: 'absolute',
    ...
    return (
        <div sx={style} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col gap-1 rounded-[12px]">
            ...
            <p className='font-[Plus Jakarta Sans] font-medium text-4 text-center'>{isRole===1?"Admin":"Sub Admin"} Deleted Successfully</p>
        </div>
    );
}

export default DeleteAdminSuccess